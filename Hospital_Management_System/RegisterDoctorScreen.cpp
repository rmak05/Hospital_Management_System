...
Register_Doctor_Screen::Register_Doctor_Screen(int w_width, int w_height) : Screen(ScreenId::register_doctor, w_width, w_height) {
	...
	add_entity(std::make_shared<TextBox>(std::string("Register Doctor"), 50u, 0.0f, sf::Vector2f(1300.0f, 100.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f), 100.0f), sf::Color::White, darkYellow, bgWhite));
	...
	add_entity(std::make_shared<Button>(std::string("Register"), 30u, OutlineThickness, sf::Vector2f(200.0f, 80.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 200.0f), 960.0f), sf::Color::Black, lightYellow, sf::Color::Black, ScreenId::manage_doctor, FuncType::add_doctor));
...
std::vector<std::string> Register_Doctor_Screen::extract_form(sf::Vector2f mouse_pos) {
...
void Register_Doctor_Screen::fill_form(std::vector<std::string>& data) {
