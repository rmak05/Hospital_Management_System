...
Patient_Details_Home_Screen::Patient_Details_Home_Screen(int w_width, int w_height) : Screen(ScreenId::patient_details_home, w_width, w_height) {
	...
	add_entity(std::make_shared<Button>(std::string(""), 50u, OutlineThickness, sf::Vector2f(400.0f, 500.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1500.0f) + 550.0f, 400.0f), sf::Color::White, bgBlue, sf::Color::Black, ScreenId::test_history));
	add_entity(std::make_shared<Button>(std::string(""), 50u, OutlineThickness, sf::Vector2f(400.0f, 500.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1500.0f) + 1100.0f, 400.0f), sf::Color::White, bgBlue, sf::Color::Black, ScreenId::appointment_history));
