...
std::vector<std::string> Appointment_Screen::extract_form(sf::Vector2f mouse_pos) {
	...
	for (entity_ptr _entity : all_entities) {
		if ((_entity->type == EntityType::button) && (_entity->isMouseHover(mouse_pos))) {
			...
			std::string patient_id = all_entities[index + 2]->getText();
...
void Appointment_Screen::erase_form() {
	// change this to 3 if image is added
	while(all_entities.size() > 2) all_entities.pop_back();
}

void Appointment_Screen::fill_form(std::vector<std::string>& data) {
	...
	for (int i = 0; i < dsize; i++) {
		...
		add_entity(std::make_shared<Button>(std::string(""), 30u, 8.0f, sf::Vector2f(1020.0f, 145.0f), sf::Vector2f(xcoord, ycoord), sf::Color::Black, lightBlueConst, lightGrey, ScreenId::patient_appointment, FuncType::get_appointment_patient_data));
		...
		add_entity(std::make_shared<TextBox>(std::string("Patient ID : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 513, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite, false));
		add_entity(std::make_shared<TextBox>(std::string("Appointment Date : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 7, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite, false));
		add_entity(std::make_shared<TextBox>(std::string("Time : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 513, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite, false));
...
void Appointment_Screen::scroll_entities(int delta) {
