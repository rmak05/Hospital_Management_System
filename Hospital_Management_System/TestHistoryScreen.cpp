...
Test_History_Screen::Test_History_Screen(int w_width, int w_height) : Screen(ScreenId::test_history, w_width, w_height) {
	...
	add_entity(std::make_shared<TextBox>(std::string("Test History"), 50u, 0.0f, sf::Vector2f(1300.0f, 100.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f), 100.0f), sf::Color::White, darkYellow, sf::Color::White));
	...
	//add_entity(std::make_shared<Button>(std::string("Click Me"), 30u, OutlineThickness, sf::Vector2f(200.0f, 80.0f), sf::Vector2f(get_center_coord(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f) + 150.0f, 700.0f, 200.0f), 700.0f), sf::Color::Black, lightYellow, sf::Color::Black, ScreenId::test_results));
}

std::vector<std::string> Test_History_Screen::extract_form(sf::Vector2f mouse_pos) {
...
void Test_History_Screen::erase_form() {
	while(all_entities.size() > 2) all_entities.pop_back();
}

void Test_History_Screen::fill_form(std::vector<std::string>& data) {
...
void Test_History_Screen::scroll_entities(int delta) {
