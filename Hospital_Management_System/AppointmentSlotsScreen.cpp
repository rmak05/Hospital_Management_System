...
Appointment_Slots_Screen::Appointment_Slots_Screen(int w_width, int w_height) : Screen(ScreenId::appointment_slots, w_width, w_height) {
	...
	add_entity(std::make_shared<TextBox>(std::string("Available Time SLots"), 50u, 0.0f, sf::Vector2f(1300.0f, 100.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f), 100.0f), sf::Color::White, darkYellow, sf::Color::White));
...
std::vector<std::string> Appointment_Slots_Screen::extract_form(sf::Vector2f mouse_pos) {
	std::vector<std::string> form_details;
	int index = 0;

	form_details.push_back(doctor_id);
	...
	for (entity_ptr _entity : all_entities) {
		if ((_entity->type == EntityType::button) && (_entity->isMouseHover(mouse_pos))) {
			...
			std::string _date = all_entities[index + 1]->getText();
			form_details.push_back(_date.substr(7));
			std::string _time = all_entities[index + 2]->getText();
			form_details.push_back(_time.substr(7));
			std::string _room_id = all_entities[index + 3]->getText();
			form_details.push_back(_room_id.substr(10));
...
void Appointment_Slots_Screen::erase_form() {
	while(all_entities.size() > 2) all_entities.pop_back();
}

void Appointment_Slots_Screen::fill_form(std::vector<std::string>& data) {
	int dsize = ((int)data.size()) - 3;
	float xcoord = get_center_coord(LEFT_MARGIN, (window_width - 2 * LEFT_MARGIN) * 1.0f, 1020.0f);
	float ycoord = 250.0f;

	doctor_id = data[dsize + 3 - 3];
	patient_id = data[dsize + 3 - 2];

	for (int i = 0; i < dsize; i++) {
		curr_entity_index = 1;

		add_entity(std::make_shared<Button>(std::string(""), 30u, 8.0f, sf::Vector2f(1020.0f, 145.0f), sf::Vector2f(xcoord, ycoord), sf::Color::Black, lightBlueConst, lightGrey, ScreenId::approve_appointment, FuncType::confirm_appointment));

		add_entity(std::make_shared<TextBox>(std::string("Date : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 7, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite, false));
		add_entity(std::make_shared<TextBox>(std::string("Time : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 513, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite, false));
...
void Appointment_Slots_Screen::scroll_entities(int delta) {
