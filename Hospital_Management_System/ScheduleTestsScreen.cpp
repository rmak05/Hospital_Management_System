...
Schedule_Tests_Screen::Schedule_Tests_Screen(int w_width, int w_height) : Screen(ScreenId::schedule_tests, w_width, w_height) {
	...
	add_entity(std::make_shared<TextBox>(std::string("Schedule Tests"), 50u, 0.0f, sf::Vector2f(1300.0f, 100.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f), 100.0f), sf::Color::White, darkYellow, sf::Color::White));
...
std::vector<std::string> Schedule_Tests_Screen::extract_form(sf::Vector2f mouse_pos) {
...
void Schedule_Tests_Screen::erase_form() {
	while(all_entities.size() > 2) all_entities.pop_back();
}

void Schedule_Tests_Screen::fill_form(std::vector<std::string>& data) {
	...
	for (int i = 0; i < dsize; i++) {
		...
		add_entity(std::make_shared<Button>(std::string(""), 30u, 8.0f, sf::Vector2f(1020.0f, 145.0f), sf::Vector2f(xcoord, ycoord), sf::Color::Black, lightBlueConst, lightGrey, ScreenId::test_slots, FuncType::get_test_slots));
		...
		add_entity(std::make_shared<TextBox>(std::string("Test : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 513, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite));
...
void Schedule_Tests_Screen::scroll_entities(int delta) {
