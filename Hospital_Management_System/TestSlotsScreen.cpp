...
Test_Slots_Screen::Test_Slots_Screen(int w_width, int w_height) : Screen(ScreenId::test_slots, w_width, w_height) {
...
std::vector<std::string> Test_Slots_Screen::extract_form(sf::Vector2f mouse_pos) {
	std::vector<std::string> form_details;
	int index = 0;

	form_details.push_back(test_id);
...
void Test_Slots_Screen::erase_form() {
	while(all_entities.size() > 2) all_entities.pop_back();
}

void Test_Slots_Screen::fill_form(std::vector<std::string>& data) {
	int dsize = ((int)data.size()) - 2;
	float xcoord = get_center_coord(LEFT_MARGIN, (window_width - 2 * LEFT_MARGIN) * 1.0f, 1020.0f);
	float ycoord = 250.0f;

	test_id = data[dsize + 2 - 2];

	for (int i = 0; i < dsize; i++) {
		curr_entity_index = 1;

		add_entity(std::make_shared<Button>(std::string(""), 30u, 8.0f, sf::Vector2f(1020.0f, 145.0f), sf::Vector2f(xcoord, ycoord), sf::Color::Black, lightBlueConst, lightGrey, ScreenId::approve_test, FuncType::confirm_test));
...
void Test_Slots_Screen::scroll_entities(int delta) {
