...
Admin_Login_Screen::Admin_Login_Screen(int w_width, int w_height) : Screen(ScreenId::admin_login, w_width, w_height) {
	...
	add_entity(std::make_shared<ImageBox>(std::string("./Images/demo.png"), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f) + 900.0f, 400.0f), sf::Vector2f(0.2f, 0.2f)));
}