...
Doctor_Home_Screen::Doctor_Home_Screen(int w_width, int w_height) : Screen(ScreenId::doctor_home, w_width, w_height) {
	...
	add_entity(std::make_shared<Button>(std::string(""), 50u, OutlineThickness, sf::Vector2f(300.0f, 500.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1500.0f) + 800.0f, 400.0f),  sf::Color::White, bgBlue, sf::Color::Black, ScreenId::my_information, FuncType::get_doctor_data));
