...
Manage_Doctor_Screen::Manage_Doctor_Screen(int w_width, int w_height) : Screen(ScreenId::manage_doctor, w_width, w_height) {
	...
	add_entity(std::make_shared<TextBox>(std::string("Manage Doctors"), 50u, 0.0f, sf::Vector2f(1300.0f, 100.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f), 100.0f), sf::Color::White, darkYellow, sf::Color::White));
	...
	add_entity(std::make_shared<Button>(std::string(""), 50u, OutlineThickness, sf::Vector2f(300.0f, 500.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1500.0f) + 1200.0f, 400.0f), sf::Color::White, bgBlue, sf::Color::Black, ScreenId::view_doctor_filters));
	...
	add_entity(std::make_shared<TextBox>(std::string("View Doctors"),      30u, OutlineThickness, sf::Vector2f(250.0f, 80.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1500.0f) + 1250.0f - 25.0f, 770.0f), sf::Color::Black, darkYellow, sf::Color::Black));
	...
	add_entity(std::make_shared<ImageBox>(std::string("./Images/admin.png"),     sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1500.0f) + 1280.0f, 500.0f), sf::Vector2f(1.0f, 1.0f)));
}