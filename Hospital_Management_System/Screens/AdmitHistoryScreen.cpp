...
void Admit_History_Screen::erase_form() {
	while(all_entities.size() > 2) all_entities.pop_back();
}

void Admit_History_Screen::fill_form(std::vector<std::string>& data) {
	...
	for (int i = 0; i < dsize; i++) {
		...
		add_entity(std::make_shared<TextBox>(std::string(""), 30u, 8.0f, sf::Vector2f(1020.0f, 145.0f), sf::Vector2f(xcoord, ycoord), sf::Color::Black, lightBlueConst, bgWhite));

		add_entity(std::make_shared<TextBox>(std::string("Room ID : ") + data[i++], 30u, 0.0f, sf::Vector2f(244.0f, 65.0f), sf::Vector2f(xcoord + 7, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite, false));
		add_entity(std::make_shared<TextBox>(std::string("Admit Date : ") + data[i++], 30u, 0.0f, sf::Vector2f(399.0f, 65.0f), sf::Vector2f(xcoord + 253, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite));
		add_entity(std::make_shared<TextBox>(std::string("Admit Time : ") + data[i++], 30u, 0.0f, sf::Vector2f(359.0f, 65.0f), sf::Vector2f(xcoord + 654, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite));
		add_entity(std::make_shared<TextBox>(std::string("Discharge Date : ") + data[i++], 30u, 0.0f, sf::Vector2f(334.0f, 65.0f), sf::Vector2f(xcoord + 7, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite, false));
		add_entity(std::make_shared<TextBox>(std::string("Discharge Time : ") + data[i++], 30u, 0.0f, sf::Vector2f(334.0f, 65.0f), sf::Vector2f(xcoord + 513, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite, false));
...
void Admit_History_Screen::scroll_entities(int delta) {
	...
	for (int i = 1; i <= entity_count; i++) {
		if ((curr_entity_index <= i) && (i <= curr_entity_index + 3)) {
			...
			all_entities[1 + (i - 1) * 6 + 3]->setPosition(sf::Vector2f(xcoord + 253, ycoord + 5));
			all_entities[1 + (i - 1) * 6 + 4]->setPosition(sf::Vector2f(xcoord + 654, ycoord + 5));
			all_entities[1 + (i - 1) * 6 + 5]->setPosition(sf::Vector2f(xcoord + 7, ycoord + 75));
			all_entities[1 + (i - 1) * 6 + 6]->setPosition(sf::Vector2f(xcoord + 513, ycoord + 75));
		}
		else {
