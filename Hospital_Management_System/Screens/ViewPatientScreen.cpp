...
View_Patient_Screen::View_Patient_Screen(int w_width, int w_height) : Screen(ScreenId::view_patient, w_width, w_height) {
...
std::vector<std::string> View_Patient_Screen::extract_form(sf::Vector2f mouse_pos) {
...
void View_Patient_Screen::erase_form() {
	while(all_entities.size() > 2) all_entities.pop_back();
}

void View_Patient_Screen::fill_form(std::vector<std::string>& data) {
	...
	for (int i = 0; i < dsize; i++) {
		...
		add_entity(std::make_shared<TextBox>(std::string("Patient ID : ") + data[i++], 25u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 7, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite, false));
		add_entity(std::make_shared<TextBox>(std::string("Disease : ") + data[i++], 25u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 513, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite, false));
		...
		add_entity(std::make_shared<TextBox>(std::string("Room ID : ") + data[i++], 25u, 0.0f, sf::Vector2f(334.0f, 65.0f), sf::Vector2f(xcoord + 679, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite, false));
...
void View_Patient_Screen::scroll_entities(int delta) {
