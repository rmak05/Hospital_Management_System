...
Data_Entry_Login_Screen::Data_Entry_Login_Screen(int w_width, int w_height) : Screen(ScreenId::dataentry_login, w_width, w_height) {
	...
	add_entity(std::make_shared<Button>(std::string("Login"), 30u, OutlineThickness, sf::Vector2f(200.0f, 80.0f), sf::Vector2f(get_center_coord(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f) + 150.0f, 700.0f, 200.0f), 700.0f), sf::Color::Black, lightYellow, sf::Color::Black, ScreenId::dataentry_login_patient, FuncType::check_dataentry_login));
