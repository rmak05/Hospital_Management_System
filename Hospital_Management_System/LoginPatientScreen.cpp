...
Login_Patient_Screen::Login_Patient_Screen(int w_width, int w_height) : Screen(ScreenId::patient_login, w_width, w_height) {
	...
	add_entity(std::make_shared<ImageBox>(std::string("./Images/demo.png"), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f) + 900.0f, 500.0f), sf::Vector2f(0.3f, 0.3f)));
}