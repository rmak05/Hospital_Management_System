...
Delete_Doctor_Screen::Delete_Doctor_Screen(int w_width, int w_height) : Screen(ScreenId::delete_doctor, w_width, w_height) {
	...
	add_entity(std::make_shared<TextBox>(std::string("Delete Doctor"), 50u, 0.0f, sf::Vector2f(1300.0f, 100.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f), 100.0f), sf::Color::White, darkYellow, sf::Color::White));
	...
	add_entity(std::make_shared<Button>(std::string("Delete"), 30u, OutlineThickness, sf::Vector2f(200.0f, 80.0f), sf::Vector2f(get_center_coord(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f) + 150.0f, 700.0f, 200.0f), 700.0f), sf::Color::Black, lightYellow, sf::Color::Black, ScreenId::manage_doctor, FuncType::delete_doctor));
