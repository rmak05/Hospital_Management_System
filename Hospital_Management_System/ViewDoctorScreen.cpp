...
View_Doctor_Screen::View_Doctor_Screen(int w_width, int w_height) : Screen(ScreenId::view_doctor, w_width, w_height) {
...
std::vector<std::string> View_Doctor_Screen::extract_form(sf::Vector2f mouse_pos) {
...
void View_Doctor_Screen::erase_form() {
	while(all_entities.size() > 2) all_entities.pop_back();
}

void View_Doctor_Screen::fill_form(std::vector<std::string>& data) {
	...
	for (int i = 0; i < dsize; i++) {
		...
		add_entity(std::make_shared<TextBox>(std::string(""), 25u, 8.0f, sf::Vector2f(1020.0f, 145.0f), sf::Vector2f(xcoord, ycoord), sf::Color::Black, lightBlueConst, sf::Color::Black));
		...
		add_entity(std::make_shared<TextBox>(std::string("Speciality : ") + data[i++], 25u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 513, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite, false));
		add_entity(std::make_shared<TextBox>(std::string("Age : ") + data[i++], 25u, 0.0f, sf::Vector2f(334.0f, 65.0f), sf::Vector2f(xcoord + 7, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite, false));
		add_entity(std::make_shared<TextBox>(std::string("Gender : ") + data[i++], 25u, 0.0f, sf::Vector2f(334.0f, 65.0f), sf::Vector2f(xcoord + 343, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite, false));
		add_entity(std::make_shared<TextBox>(std::string("Salary : ") + data[i++], 25u, 0.0f, sf::Vector2f(334.0f, 65.0f), sf::Vector2f(xcoord + 679, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite, false));
...
void View_Doctor_Screen::scroll_entities(int delta) {
	...
	for (int i = 1; i <= entity_count; i++) {
		if ((curr_entity_index <= i) && (i <= curr_entity_index + 3)) {
			...
			all_entities[1 + (i - 1) * 6 + 5]->setPosition(sf::Vector2f(xcoord + 343, ycoord + 75));
			all_entities[1 + (i - 1) * 6 + 6]->setPosition(sf::Vector2f(xcoord + 679, ycoord + 75));
		}
		else {
			...
			all_entities[1 + (i - 1) * 6 + 6]->setPosition(sf::Vector2f(xcoord, ycoord));
		}
	}
