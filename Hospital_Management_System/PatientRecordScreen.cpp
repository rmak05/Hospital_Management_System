...
Patient_Record_Screen::Patient_Record_Screen(int w_width, int w_height) : Screen(ScreenId::patient_record, w_width, w_height) {
	...
	add_entity(std::make_shared<TextBox>(std::string("Patient Records"), 50u, 0.0f, sf::Vector2f(1300.0f, 100.0f), sf::Vector2f(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f), 100.0f), sf::Color::White, darkYellow, sf::Color::White));
	...
	//add_entity(std::make_shared<Button>(std::string("Click Me"), 30u, OutlineThickness, sf::Vector2f(200.0f, 80.0f), sf::Vector2f(get_center_coord(get_center_coord(LEFT_MARGIN, (w_width - 2 * LEFT_MARGIN) * 1.0f, 1300.0f) + 150.0f, 700.0f, 200.0f), 700.0f), sf::Color::Black, lightYellow, sf::Color::Black, ScreenId::patient_details_home));
}

std::vector<std::string> Patient_Record_Screen::extract_form(sf::Vector2f mouse_pos) {
	...
	for (entity_ptr _entity : all_entities) {
		if ((_entity->type == EntityType::button) && (_entity->isMouseHover(mouse_pos))) {
			...
			std::string patient_id = all_entities[index + 1]->getText();
			form_details.push_back(patient_id.substr(13));
...
void Patient_Record_Screen::erase_form() {
	// change this to 3 if image is added
	while(all_entities.size() > 2) all_entities.pop_back();
}

void Patient_Record_Screen::fill_form(std::vector<std::string>& data) {
	...
	for (int i = 0; i < dsize; i++) {
		...
		add_entity(std::make_shared<Button>(std::string(""), 30u, 8.0f, sf::Vector2f(1020.0f, 145.0f), sf::Vector2f(xcoord, ycoord), sf::Color::Black, lightBlueConst, lightGrey, ScreenId::patient_details_home, FuncType::get_patient_data));

		add_entity(std::make_shared<TextBox>(std::string("Patient ID : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 7, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite));
		add_entity(std::make_shared<TextBox>(std::string("Patient Name : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 513, ycoord + 5), sf::Color::Black, lightBlueConst, bgWhite));
		add_entity(std::make_shared<TextBox>(std::string("Room ID : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 7, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite));
		add_entity(std::make_shared<TextBox>(std::string("Disease : ") + data[i++], 30u, 0.0f, sf::Vector2f(500.0f, 65.0f), sf::Vector2f(xcoord + 513, ycoord + 75), sf::Color::Black, lightBlueConst, bgWhite));
	...
	int entity_count = (((int)all_entities.size()) - 3) / 5;
...
void Patient_Record_Screen::scroll_entities(int delta) {
	int entity_count = (((int)all_entities.size()) - 4) / 5;
