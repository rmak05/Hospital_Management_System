...
Presc_Given_Screen::Presc_Given_Screen(int w_width, int w_height) : Screen(ScreenId::presc_given, w_width, w_height) {
	...
	add_entity(std::make_shared<TextBox>(std::string("Prescription"), 30u, 0.0f, sf::Vector2f(260.0f, 60.0f), sf::Vector2f(220.0f, 620.0f), sf::Color::Black, darkBlue, sf::Color::Black));
...
void Presc_Given_Screen::fill_form(std::vector<std::string>& data) {
	for (int i = 10; i <= 16; i++) {
		all_entities[i]->setText(data[i - 10]);
	}
}